...
export default class MemoryApp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      page: "start",
      ...
      for (let i = 0; i < this.state.numbers_count; i++) {
        line.push(
          this.getRandomIntInclusive(
            this.state.min_random,
            this.state.max_random
          )
        );
      }
  ...
  startTimer = async (t, nextPressed = 0) => {
    let timer = nextPressed === 1 ? 0 : this.state.timer,
      seconds = 0;
  ...
  result = () => {
    return this.state.result.map((l, i) => {
      return (
        <View style={styles.numbersBox} key={i}>
          {l.map((l2, i2) => {
            return (
              <Text
                style={[
                  styles.numberResult,
                  { color: !l2.correct ? "#e74c3c" : "#2ecc71" },
                ]}
                key={i2}
              >
                {l2.value}
  ...
  taskStart = (nextPressed = 0) => {
    ...
    if (
      this.state.levels < this.state.task ||
      (nextPressed == 1 && this.state.task == this.state.levels)
    ) {
      this.check();
      this.setState({
        page: "finish",
  ...
  numbersLine() {
    ...
    return this.state.numbers[this.state.task][0].map((l, i) => {
      return (
        <Text style={styles.number} key={i}>
          {l}
        </Text>
      );
    });
  ...
  variantCreate = () => {
    ...
    this.state.numbers[this.state.task][1].map((l, i) => {
      let variant_index = this.getRandomIntInclusive(
        0,
        this.state.variants_count - 1
      );
      if (!variants[variant_index]) {
        variants[variant_index] = l;
      } else {
        variants[
          this.getRandomIntInclusive(0, this.state.variants_count - 1)
        ] = l;
      }
    });
    for (let i = 0; i < this.state.variants_count; i++) {
      if (!variants[i]) {
        variants[i] = this.getRandomIntInclusive(
          this.state.min_random * 2,
          this.state.max_random * 2
        );
      }
    }
  ...
  variantRender = () => {
    return this.state.variants.map((l, i) => {
      return (
        ...
        >
          <Text style={styles.variantText} key={i}>
            {l}
          </Text>
        </TouchableHighlight>
      );
  ...
  variantPress = (value, key) => {
    ...
    this.setState({ answers: this.state.answers }, () => {
      this.setState({
        currentAnswer:
          this.state.currentAnswer === this.state.numbers_count - 2
            ? 0
  ...
  pages = () => {
    switch (this.state.page) {
        ...
        return (
          <Fragment>
            ...
            <Text style={styles.desc}>
              You will be given {this.state.numbers_count} numbers. Your task is
              to try to remember them in {this.state.time_1} seconds, then add
              the first with the second in your mind, and write down the
              resulting amount; add the second number with the third, write down
              the amount; etc. Thus, {this.state.numbers_count - 1} amounts
              should be received and recorded from you. The calculation and
              execution time is {this.state.time_2} seconds. Then proceed to the
              next row of numbers
            </Text>
            <TouchableHighlight
              underlayColor="#5cd28e"
      ...
      case "task":
        return (
          <Fragment>
            <Text style={styles.titleLevels}>
              {this.state.task + 1}/{this.state.levels + 1}
            </Text>
            <Text style={styles.title}>Memorize the numbers</Text>
            <View style={styles.numbersBoxTask}>{this.numbersLine()}</View>
          </Fragment>
        );
      ...
      case "task_sum":
        return (
          <Fragment>
            ...
            <View style={styles.numbersBox}>{this.answers()}</View>
      ...
      case "finish":
        return (
          <Fragment>
            ...
            <Text style={styles.descEnd}>
              The number of correct answers - {this.state.correct_count}. The
              norm of an adult is from{" "}
              {(this.state.numbers_count - 1) * 10 * 0.75} and above
            </Text>
            {this.result()}
            <TouchableHighlight
              underlayColor="#5cd28e"
